...
num_outputs = 10
num_hiddens = 256
W1 = torch.normal(0, 0.01, size=(num_inputs, num_hiddens), requires_grad=True, device=device)
b1 = torch.zeros(num_hiddens, requires_grad=True, device=device)
W2 = torch.normal(0, 0.01, size=(num_hiddens, num_outputs), requires_grad=True, device=device)
...
for epoch in range(EPOCH):
    ...
    for X, y in train_loader:
        ...
        X = X.reshape(-1, num_inputs)   # flatten (batch, 1, 28, 28) → (batch, 784)
        y_hat = net(X)
